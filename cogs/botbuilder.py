...
from utils.validation import safe_slug, clean_csv
...
def py_project(name: str, features: list[str]) -> dict[str, str]:
    ...
    for feature in features:
        ...
        feature_code.append(f'@bot.tree.command(name="{key}", description="Generated Forge command")\nasync def {key}(interaction: discord.Interaction):\n    await interaction.response.send_message("{feature.replace(chr(34), chr(39))[:150]}")\n')
    return {
...
def js_project(name: str, features: list[str]) -> dict[str, str]:
    ...
    for feature in features:
        key = safe_slug(feature, "feature").replace("-", "_")
        commands_code.append(f'async function {key}(interaction) {{ await interaction.reply({json.dumps(feature[:150])}); }}')
    return {
        "index.js": 'require("dotenv").config();\nconst { Client, GatewayIntentBits } = require("discord.js");\nconst client = new Client({ intents: [GatewayIntentBits.Guilds] });\nclient.once("ready", () => console.log(`Logged in as ${client.user.tag}`));\n' + '\n'.join(commands_code) + '\nclient.login(process.env.DISCORD_TOKEN);\n',
        "package.json": json.dumps({"name": safe_slug(name), "version": "1.0.0", "private": True, "scripts": {"start": "node index.js"}, "dependencies": {"discord.js": "^14.0.0", "dotenv": "^16.0.0"}}, indent=2) + "\n",
        ".env.example": "DISCORD_TOKEN=\n",
        ".gitignore": ".env\nnode_modules/\n",
...
class BotBuilder(commands.Cog):
    ...
    async def generate_bot(self, interaction, name: str, language: app_commands.Choice[str], features: str = "ping, help, moderation"):
        ...
        embed = discord.Embed(title="🤖 Bot Forge", description=f"Generated **{name[:80]}** with {len(feature_list)} requested features.", color=0x6366F1)
        embed.add_field(name="Runtime", value="discord.py" if language.value == "python" else "discord.js")
        embed.add_field(name="Included", value="ZIP project + README + manifest + secret-safe environment template", inline=False)
