...
class Database:
    ...
    def record_usage(self, guild_id, user_id, command):
        with self._connect() as con:
            con.execute(
                ...
                (int(guild_id or 0), int(user_id), str(command)),
            )
    ...
    def save_session_message(self, guild_id, user_id, session, role, content):
        with self._connect() as con:
            con.execute(
                "INSERT INTO ai_sessions(guild_id,user_id,session,role,content) VALUES(?,?,?,?,?)",
                (guild_id, user_id, session, role, content),
            )

    def get_session(self, guild_id, user_id, session, limit=12):
        with self._connect() as con:
            rows = con.execute(
                "SELECT role, content FROM ai_sessions WHERE guild_id=? AND user_id=? AND session=? ORDER BY rowid DESC LIMIT ?",
                (guild_id, user_id, session, limit),
            ).fetchall()
    ...
    def clear_session(self, guild_id, user_id, session):
        with self._connect() as con:
            con.execute(
                "DELETE FROM ai_sessions WHERE guild_id=? AND user_id=? AND session=?",
    ...
    def save_item(self, table, guild_id, user_id, name, payload, language=None):
        if table == "saved_projects":
            sql = (
                "INSERT INTO saved_projects(guild_id,user_id,name,language,payload) VALUES(?,?,?,?,?) "
                "ON CONFLICT(guild_id,user_id,name) DO UPDATE SET language=excluded.language,payload=excluded.payload,updated_at=CURRENT_TIMESTAMP"
            )
            args = (guild_id, user_id, name, language or "unknown", payload)
        elif table == "saved_embeds":
            sql = (
                "INSERT INTO saved_embeds(guild_id,user_id,name,payload) VALUES(?,?,?,?) "
                "ON CONFLICT(guild_id,user_id,name) DO UPDATE SET payload=excluded.payload,updated_at=CURRENT_TIMESTAMP"
            )
            args = (guild_id, user_id, name, payload)
        else:
            raise ValueError("Unsupported saved-data table")
        with self._connect() as con:
            con.execute(sql, args)

    def get_items(self, table, guild_id, user_id):
        if table not in {"saved_projects", "saved_embeds"}:
            raise ValueError("Unsupported saved-data table")
        with self._connect() as con:
            return con.execute(
                f"SELECT name,payload FROM {table} WHERE guild_id=? AND user_id=? ORDER BY updated_at DESC",
                (guild_id, user_id),
            ).fetchall()
